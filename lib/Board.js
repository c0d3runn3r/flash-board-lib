...
class Board {
	 ...
	 * The returned element is guaranteed to be willing to .pair() with the asset.
	 * 
	 * @param {Asset|string} [target="Element"] - The asset to create an element for, or the string name of the constructor 
	...
	element_factory(target="Element", params={}) {
		...
		} else if (typeof target === 'string') { classname = target; 
		...
		if(!element.pair(target, true)) { throw new Error(`Element class '${classname}' is not configured to accept asset of type '${target.constructor.name}'.`); }
