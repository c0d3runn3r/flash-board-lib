...
class Element extends EventEmitter {
        ...
        switch(format) {
            ...
            case 'object':
                return {
                    ...
                    static: this.static,
                    asset_type: this?.asset?.constructor.name || null,
    ...
    /**
     * Provide a condition for this element.
     * @abstract
     * @returns {ElementCondition} The condition for this element.
     */
    get condition() {
        return new ElementCondition("green", "unknown");
    }
     ...
     * 
     * @param {*} asset 
     * @throws {Error} If the asset is not an instance of Asset 
     * @return {boolean} true if the asset was accepted, false otherwise.
     */
    pair(asset, test=false) {

        // Can't pair if asset is not defined
        if(!asset ||(!asset instanceof Asset)) { throw new Error("Asset must be an instance of Asset."); }
    ...
    unpair() {
        ...
        if (!this.#asset) { throw new Error("No asset to unpair."); }
